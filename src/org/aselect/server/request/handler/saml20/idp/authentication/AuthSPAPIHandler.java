...
/**
 * This class handles authentication responses and API calls originating from an
 * authsp. It must be used as follows: <br>
 * For each new incoming request, create a new <code>AuthSPRequestHandler</code>
 * object and call its <code>handleRequest()</code> method.
 * <code>AuthSPRequestHandler</code> objects cannot be reused due to
 * concurrency issues.
 * 
 * @author Alfa & Ariss
 */
public class AuthSPAPIHandler extends AbstractAPIRequestHandler
{
		...
		throws ASelectException
	{
		...
		_systemLogger.log(Level.INFO, _sModule, sMethod, "AuthSPApiREQ " + oInputMessage);
		...
		throws ASelectCommunicationException
	{
		...
		if (!CryptoEngine.getHandle().verifySignature(sAuthSP, sSessionId, sSignature)) {
			_systemLogger.log(Level.WARNING, _sModule, sMethod, "Invalid signature from AuthSP: " + sAuthSP);
