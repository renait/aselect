package org.aselect.server.request.handler.saml20.idp.metadata;
 ...
 */
public class MetaDataRequestHandlerIDP extends AbstractRequestHandler
{
	...
	private final String artifactResolutionServiceBindingConstant = SAMLConstants.SAML2_SOAP11_BINDING_URI;
	...
	/**
	 * Init for class MetaDataRequestHandlerIDP
	 * <br>
		...
		throws ASelectException
	{
		...
		_systemLogger.log(Level.INFO, MODULE, sMethod, "metadatXML file for entityID " + entityIdIdp + " " + mdxml);
		...
		throws ASelectException
	{
		...
		try {
			...
			while (enumAliases.hasMoreElements()) {
				...
				if (sAlias.equals(publicKeyAlias)) { //server_id van aselectidp xml federation-idp
	...
	private String createMetaDataXML()
	{
		...
		String ArtifactResolutionServiceLocation = redirectURL + artifactresolverTarget; // "http://localhost:8080/aselect_idp/server/SAML/Artifact/Resolve";
		String singleLogoutServiceLocationSoap = redirectURL + singleLogoutRequestTarget;
		...
		String xmlMDRequest = "<?xml version=\"1.0\"?>"
				...
				+ entityIdIdp
				+ "\">"
				+ "	<m:IDPSSODescriptor WantAuthnRequestsSigned=\"false\"  protocolSupportEnumeration=\"urn:oasis:names:tc:SAML:2.0:protocol\">"
				...
				+ artifactResolutionServiceBindingConstant + "\"" + " Location=\"" + ArtifactResolutionServiceLocation
				...
				+ singleLogoutServiceLocationSoap + "\">" + "</m:SingleLogoutService>"
				+ "<m:SingleSignOnService Binding=\"" + singleSignOnServiceBindingConstantREDIRECT + "\""
				+ " Location=\"" + singleSignOnServiceLocation + "\">" + "</m:SingleSignOnService>"
				+ "</m:IDPSSODescriptor>" + "</m:EntityDescriptor>";
		return xmlMDRequest;
	}

	/**
	 * Process request for meta dataP
	 * <br>
