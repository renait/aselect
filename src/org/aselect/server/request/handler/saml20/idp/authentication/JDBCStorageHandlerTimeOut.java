...
public class JDBCStorageHandlerTimeOut extends JDBCStorageHandler
{
		...
		throws ASelectStorageException
	{
		...
		long now = new Date().getTime();
		_oSystemLogger.log(Level.INFO, MODULE, _sMethod, "IDPTO now="+now);
		...
		try {
			...
			if (allTgts == null)
				return;
			_oSystemLogger.log(Level.INFO, MODULE, _sMethod, "IDPTO - TGT Count=" + allTgts.size());
			...
			for (Enumeration<String> e = allTgts.keys(); e.hasMoreElements();) {
				...
				_oSystemLogger.log(Level.INFO, MODULE, _sMethod, "ExpInt="+lExpInterval+
						" Timeout="+timeOutTime+" Create="+(lCreateTime-now) + " Danish="+danishLogout);
				for (ServiceProvider sp : spList) {
					_oSystemLogger.log(Level.INFO, MODULE, _sMethod, " IDPTO - Uid=" + uid +
				...
				for (ServiceProvider sp : spList) {
					...
					_oSystemLogger.log(Level.INFO, MODULE, _sMethod, "IDPTO - ListSize="+spList.size()+
