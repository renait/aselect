...
public class Xsaml20_TokenRequestHandler extends Saml20_BaseHandler
{
	...
	throws IOException, ASelectException
	{
		...
		try {
			...
			parms.putAll(servletRequest.getParameterMap());
			// RM_62_05
			...
			if (PARM_VALUE_ATRRIBUTESTATEMENT.equalsIgnoreCase(samlrequest)) {
				...
				if (createtgt != null && createtgt.equalsIgnoreCase("true")) {
					// RM_62_07
					String uid = servletRequest.getParameter(PARM_NAME_UID);
			...
			if (PARM_VALUE_ENCODING_BASE64.equalsIgnoreCase(servletRequest.getParameter(PARM_NAME_ENCODING))) {
				servletResponse.setContentType("application/octet-stream");
				BASE64Encoder b64enc = new BASE64Encoder();
				returnstring = b64enc.encode(returnstring.getBytes("UTF-8"));
			}
			else {
				servletResponse.setContentType("application/samlassertion+xml");
			}
