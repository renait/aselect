...
public abstract class SamlAssertionConsumer extends ProtoRequestHandler
{
	...
	throws ASelectException
	{
		...
		String sArtifactUrl;
		_systemLogger.log(Level.INFO, MODULE, sMethod, "Query=" + request.getQueryString() + ", RedirectUrl="
				+ sRedirectUrl);
		if (sRedirectUrl == null)
			sRedirectUrl = "no-application";
		...
		try {
			...
			oSamlRequest.sign(XMLSignature.ALGO_ID_SIGNATURE_RSA_SHA1, _configManager.getDefaultPrivateKey(),
			...
			if (sStatus != null && sAssertion != null && sStatus.contains("samlp:Success")) {
				...
				_systemLogger.log(Level.INFO, MODULE, sMethod, "sAppId=" + _sMyAppId + " sTarget=" + sRedirectUrl
						+ " _sMyOrg=" + _sMyOrg);
	...
	// NOT USED
	/*
	 * private String getAttrFromSaml(String sName, String sAssertion) { String sMethod = "getAttrFromSaml"; int nIdx =
	 * sAssertion.indexOf("AttributeName=\""+sName+"\""); String sUserId = null; if (nIdx >= 0) sUserId =
	 * Tools.extractFromXml(sAssertion.substring(nIdx), "AttributeValue", true); if (sUserId == null) {
	 * _systemLogger.log(Level.WARNING, MODULE, sMethod, "Attribute '"+sName+"' not found"); return null; } return
	 * sUserId; }
	 */
}
