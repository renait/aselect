...
public class XSAML11RequestHandler extends ProtoRequestHandler
{
	...
	throws ASelectException
	{
		...
		try {
			...
			/*
			 * else if (_iArtifactType == 2) { URI oURI = new URI(_sSourceLocation); // communicate our SAML responder
			 * URI _systemLogger.log(Level.INFO, MODULE, sMethod, "_sSourceLocation="+_sSourceLocation); oSAMLArtifact =
			 * new SAMLArtifactType0002(oURI); }
			 */
			...
			_systemLogger.log(Level.INFO, MODULE, sMethod, "sIP=" + sIP + ", sHost=" + sHost + ", htResponse="
					+ htResponse);
			SAMLAssertion oSAMLAssertion = _saml11Builder.createSAMLAssertionFromCredentials(sUid, null, null, sIP,
					sHost, SAMLSubject.CONF_ARTIFACT, sProviderId, null/* audience */, htResponse);
