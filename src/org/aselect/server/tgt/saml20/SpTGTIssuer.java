...
import org.aselect.server.request.handler.saml20.sp.authentication.UserToTgtMapper;
 ...
 * @author Alfa & Ariss
 */
public class SpTGTIssuer
{
	/**
	 * The name of this module, that is used in the system logging.
	 */
	private static final String MODULE = "saml20.SpTGTIssuer";
	...
	public static final String COOKIE_NAME = "aselect_credentials";  // "pip_sp_credentials";
	 ...
	 */
	public SpTGTIssuer(String sServerId) {
		// TODO All configuration reading in this class must be moved to this
		// contructor (Martijn)
	 ...
	 *            <code>Hashtable</code> containing additional TGT information
	 * @param oHttpServletResponse
	 *            The servlet response that is used to redirect to
	 * @param sOldTGT
	 *            The aselect_credentials_tgt that is already set as a cookie at
	 *            the user (can be null if not present)
		...
		throws ASelectException
	{
		...
		try {
			...
			_systemLogger.log(Level.INFO, MODULE, sMethod, "SP TGT: " + sTgt);
			...
			while (keys.hasMoreElements()) {
				...
				_systemLogger.log(Level.FINEST, MODULE, sMethod, "     " + key + " = " + sValue);
			}*/

			// put tgt in userToTgtMapper
			UserToTgtMapper.put(sUserId, sTgt);
	 ...
	 * @param sOldTGT
	 *            The aselect_credentials_tgt that is already set as a cookie at
	 *            the user (can be null if not exists)
	 * @throws ASelectException
	 *             if an error page must be shown
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * authentication. This error code will be returned to the web application
	 * during the verify_credentials API call. <br>
	 * <br>
	 * <b>Description:</b>
	 * <ul>
	 * <li>Creates a specific redirect url</li>
	 ...
	 *            The request id (session key)
	 * @param sResultCode
	 *            The error code that occurred and will be returned to the
		...
		throws ASelectException
	{
		...
		try {
			...
			String sEncryptedTgt = _cryptoEngine.encryptTGT(Utils.stringToHex(sTgt));
	...
	/*
	 * sets the A-Select Cookie (aselect_credentials) containing the A-Select
	 * credentials
	 */
		...
		throws ASelectException
	{
		...
		try {
			...
			Cookie oCredentialsCookie = new Cookie(COOKIE_NAME, sbCredentials.toString());
	...
	/*
	 * verifies the old tgt with new tgt context verifies the following items: -
	 * app_level
	 */
	@SuppressWarnings("unchecked")
	private Hashtable verifyTGT(Hashtable htOldTGTContext, Hashtable htNewTGTContext)
	{
		...
		// only if the application requires forced this is usefull
