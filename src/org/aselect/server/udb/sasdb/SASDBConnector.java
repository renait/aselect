...
public class SASDBConnector implements IUDBConnector
{
	 ...
	 * - The udb storage which optional
	 * - No filter has to be configured
	 * - Every filter must have an unique id, starting with 1 and increased by one
	 * - One or more authsps must be configured were the user attribute will be generated for
	 * <br><br>
	...
	{	
        ...
        try
        {
	        ...
	        if (oUdbStorage != null)
	        {
		        ...
		        {
		            _oASelectSystemLogger.log(Level.CONFIG, MODULE, sMethod, "No valid 'connector' config section found in 'udb_storage'", e);
		        ...
		        {
		            _oASelectSystemLogger.log(Level.CONFIG, MODULE, sMethod, "No valid 'udb' config section found in A-Select config", e);
		            throw new ASelectUDBException(Errors.ERROR_ASELECT_INIT_ERROR, e);
		        }
		        
		        try
		        {
		            oUdbStorageSection = oASelectConfigManager.getSection(oUdbSection, "connector", "id=" + sUdbStorageConnector);
		        }
		        catch (ASelectConfigException e)
		        {
		        ...
		        {
		            StringBuffer sbFailed = new StringBuffer(
		                "Invalid 'class' config item isn't a valid class: ");
		        ...
		        {
		            StringBuffer sbFailed = new StringBuffer(
		                "Could not initialize the configured UDB Connector: ");
		            sbFailed.append(sStorageClass);
	        ...
	        try
	        {
	            oFiltersSection = oASelectConfigManager.getSection(
	                oConfigSection, "filters");
	        }
	        catch (Exception e)
	        {
	        ...
	        try
	        {
	            oFilterSection = oASelectConfigManager.getSection(
	                oFiltersSection, "filter");
	        }
            catch (Exception e)
            {
                oFilterSection = null;
                
	            _oASelectSystemLogger.log(Level.CONFIG, MODULE, sMethod, "No valid 'filter' config section found in 'filters' section, disabling filters", e);
            }
            ...
            while (oFilterSection != null)
            {
                ...
                {
                    StringBuffer sbFailed = new StringBuffer(
                        "'id' config item is not an integer value: ");
                    sbFailed.append(sID);
		...
		try
		{
	    	...
	    	if (htResponse.isEmpty())
	    	{
	    	    ...
	    	    { 
	    	        ...
	    	        if (sPattern == null)
	    	        {
	    	            StringBuffer sbBuffer = new StringBuffer("SASDB filters are not configured in a following order.");
	                    _oASelectSystemLogger.log(Level.CONFIG, 
        	...
        	if (!bEnabled)
        	{
            	...
            	if(!bEnabled)
            	{
    	            ...
    	            StringBuffer sbBuffer = new StringBuffer(
    	                "User is not allowed to use the SASDBHandler: '");
	...
	private boolean matchFilter(String sData, String sFilterPattern)
	{
		...
		StringCharacterIterator iter =
			new StringCharacterIterator(sFilterPattern);
		
		for (c = iter.first(); c != StringCharacterIterator.DONE &&
			i < sData.length(); c = iter.next() )
		{
			if (c == '?')
				i++;
			else if (c == '*')
			{
				...
				{
					if (matchFilter(sData.substring(i),
							sSubFilter)) return true;
					i++;
				}
				return false;
			}
