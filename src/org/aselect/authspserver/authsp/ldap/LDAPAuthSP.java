...
{       
    ...
    /** The version. */
    public static final String VERSION = "A-Select LDAP AuthSP 1.7";
        ...
        try
        {
            ...
            if (_cryptoEngine == null)
            {
                _systemLogger.log(Level.WARNING, MODULE, sMethod, 
                    "No CryptoEngine found in servlet context.");                
            ...
            {
                _systemLogger.log(Level.WARNING, MODULE, sMethod, 
                    "No 'friendly_name' found in servlet context.");
                throw new ASelectException(Errors.ERROR_LDAP_INTERNAL_ERROR);
            }
            _systemLogger.log(Level.INFO, MODULE, sMethod, 
            ...
            {
                _systemLogger.log(Level.WARNING, MODULE, sMethod, 
                    "No working_dir found in servlet context.");
                throw new ASelectException(Errors.ERROR_LDAP_INTERNAL_ERROR);
            }            
            _systemLogger.log(Level.INFO, MODULE, sMethod, 
            ...
            try
            {
                _oAuthSpConfig = _configManager.getSection(null, "authsp", 
                    "id=" + sConfigID);
            }
            ...
            try
            {
                String sAllowedRetries =  _configManager.getParam(_oAuthSpConfig, 
                    "allowed_retries");
       ...
       throws java.io.IOException 
    {
        ...
        try
        {
            ...
            _systemLogger.log(Level.INFO, MODULE, sMethod, "LDAP GET {"+
            		servletRequest+" --> "+sMethod+", "+((sRequestName!=null)?sRequestName:"NULL")+": "+
            		sQueryString);
           
            if(sRequestName != null) //API request
            {
                handleApiRequest(htServiceRequest,servletRequest, pwOut,
                    servletResponse);               
            }
            else //Browser request
            {
                ...
                if ((sRid == null) || (sAsUrl == null) || 
                    (sUid == null) || (sAsId == null) || (sSignature == null))
                {
                    _systemLogger.log(Level.WARNING, MODULE, sMethod,
                ...
                if (!_cryptoEngine.verifySignature(sAsId, 
                    sbSignature.toString(), sSignature))
                {
                ...
                LDAPProtocolHandlerFactory.getContext(
        ...
        throws java.io.IOException 
    {
        ...
        try
        {
            ...
            _systemLogger.log(Level.INFO, MODULE, sMethod, "LDAP POST {"+
            		servletRequest+" --> "+sMethod+", "+sRid+": "+ sMyUrl);
            ...
            if (sPassword.trim().length() < 1) //invalid password
            {
                ...
                showAuthenticateForm(pwOut, Errors.ERROR_LDAP_INVALID_PASSWORD, 
                    _configManager.getErrorMessage(
                        Errors.ERROR_LDAP_INVALID_PASSWORD, 
                        _oErrorProperties), htServiceRequest);
            }
            else
            {                         
                //generate signature            
                ...
                if (!_cryptoEngine.verifySignature(
                            sAsId, sbSignature.toString(), 
                            URLDecoder.decode(sSignature, "UTF-8")))
                {
                    StringBuffer sbWarning = new StringBuffer("Invalid signature from A-Select Server '");
                    sbWarning.append(sAsId);
                    sbWarning.append("' for user: ");
                ...
                ILDAPProtocolHandler oProtocolHandler = 
                        LDAPProtocolHandlerFactory.instantiateProtocolHandler(
                            _oAuthSpConfig, sUid, _systemLogger);
                   ...
                   //invalid password
                {
                    ...
                    if (iRetriesDone < _iAllowedRetries) //try again
                    {
                        ...
                        htServiceRequest.put("rid", sRid);
                        htServiceRequest.put("a-select-server", sAsId);
                        htServiceRequest.put("retry_counter", 
                            String.valueOf(iRetriesDone + 1));
                        htServiceRequest.put("signature", sSignature);
                        if (sCountry != null)
                            htServiceRequest.put("country", sCountry);
                        if (sLanguage != null)
                    ...
                    else 
                    {
                        ...
                        _authenticationLogger.log(new Object[] { 
                            ...
                            "denied"});
                        handleResult(
                            servletRequest, servletResponse, pwOut, sResultCode);
                    }
                }
                ...
                {
                    _systemLogger.log(Level.WARNING, MODULE, sMethod,
                        "Error authenticating user, cause: " + sResultCode);
                    handleResult(
                        servletRequest, servletResponse, pwOut, sResultCode);
                }
            }
        ...
        catch(ASelectException eAS)
        {
            _systemLogger.log(Level.WARNING, MODULE, sMethod, 
                "Sending error to client", eAS);
            handleResult(
                servletRequest, servletResponse, pwOut, eAS.getMessage()); 
        }
        ...
        catch(NumberFormatException eNF) //error parsing retry_counter
        {
            ...
            handleResult(servletRequest, servletResponse, pwOut, 
                Errors.ERROR_LDAP_INVALID_REQUEST);  
        }
...
//      TODO Restart functionality has to be added (Erwin)
                              ...
                              String sResultCode) 
    {
        ...
        {
            if (_sFailureHandling.equalsIgnoreCase("aselect") || 
                sResultCode.equals(Errors.ERROR_LDAP_SUCCESS))
            ...
            {           
                ...
                if(sRid == null || sAsUrl == null || sAsId == null)
                {
                    showErrorPage(pwOut, _sErrorHtmlTemplate, sResultCode, 
                        _configManager.getErrorMessage(sResultCode, 
                            _oErrorProperties)); 
                }
                else
                {
                    ...
                    catch(IOException eIO)  //could not send redirect
                    {
                        StringBuffer sbError = new StringBuffer(
                            "Could not send redirect to: \"");
                        sbError.append(sbTemp.toString()).append("\"");
                        _systemLogger.log(Level.WARNING, MODULE, sMethod,
                            sbError.toString(), eIO);
                    }
                }
            }
        ...
        {
           _systemLogger.log(Level.WARNING, MODULE, sMethod,
               "Could not generate LDAP AuthSP signature",eAS);
           showErrorPage(pwOut, _sErrorHtmlTemplate, 
               Errors.ERROR_LDAP_COULD_NOT_AUTHENTICATE_USER,
               _configManager.getErrorMessage(sResultCode, _oErrorProperties));
        }
        ...
        {
            _systemLogger.log(Level.WARNING, MODULE, sMethod,
                "Could not encode LDAP AuthSP signature", eUE);
            showErrorPage(pwOut, _sErrorHtmlTemplate, 
                Errors.ERROR_LDAP_COULD_NOT_AUTHENTICATE_USER,
                _configManager.getErrorMessage(sResultCode, _oErrorProperties));
        }        
     ...
     */
    private void handleApiRequest(Hashtable htServiceRequest,
        ...
        HttpServletResponse servletResponse)
    {
        ...
        {        
                ...
                //authenticate request
            {
                ...
                {
                    _systemLogger.log(Level.WARNING, MODULE, sMethod, 
                    "No login retries left for rid: '"+sRid+"'");
     ...
     */
    private void handleAuthenticate(Hashtable htServiceRequest, 
        HttpServletRequest servletRequest) throws ASelectException
    {
        ...
        {
            _systemLogger.log(Level.WARNING, MODULE, sMethod, 
                "Invalid API request: invalid password.");
            throw new ASelectException(Errors.ERROR_LDAP_INVALID_REQUEST);
        }
        ...
        try
        {
            ...
            //authenticate user
            ILDAPProtocolHandler oProtocolHandler = 
            LDAPProtocolHandlerFactory.instantiateProtocolHandler(_oAuthSpConfig, sUid, _systemLogger);                                     
            String sResultCode = oProtocolHandler.authenticate(sPassword);
            ...
            {
                _systemLogger.log(Level.WARNING, MODULE, sMethod, 
                    "Could not authenticate user, cause:" + sResultCode);
                throw new ASelectException(sResultCode);
            }
        }
        ...
        {
            _systemLogger.log(Level.WARNING, MODULE, sMethod , 
                "Could not decode request parameter",eUE);
            throw new ASelectException(Errors.ERROR_LDAP_INTERNAL_ERROR);
        }
    }
     ...
     * 
     */
    private void showAuthenticateForm(PrintWriter pwOut,
                                      String sError,
                                      String sErrorMessage,
                                      Hashtable htServiceRequest)
    {
        ...
        _systemLogger.log(Level.INFO, MODULE, "showAuthenticateForm", "FORM "+_sWorkingDir+"/"+"authenticate.html, friendly="+_sFriendlyName); 
