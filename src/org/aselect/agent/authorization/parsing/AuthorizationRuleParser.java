 ...
 * <br><br>
 * <b>Description:</b><br>
 * This LL(1) Parser uses a {@link AuthorizationRuleScanner} to parse
 * authorization rules.
 * <br><br>
 * <b>Concurrency issues:</b>
 * <br>
 * Use one <code>AuthorizationRuleParser</code> per authorization rule.
 * <br>
 * @author Alfa & Ariss
 * 
 */
...
{   
    ...
    /**
     * Create a new <code>AuthorizationRuleParser</code>.
     * <br><br>
     * <b>Description:</b>
     * <br>
     * Create a new <code>AuthorizationRuleParser</code> with the given 
     * <code>SystemLogger</code> and using the given 
     * <code>AuthorizationRuleScanner</code>.
     * <br><br>
     * <b>Concurrency issues:</b>
     * <br>
     * Use one <code>AuthorizationRuleParser</code> per authorization rule.
     ...
     * <br>
     * @param oScanner The scanner which scans the authorization rule.
     * @param systemLoger The logger for system entries.
     */
     ...
     * <b>Postconditions:</b>
     * <br>
     * The evaluation tree is parsed and the {@link #getEvaluationTree} 
     * method returns the parsed evaluation rule.
     * <br>
     * @throws ASelectAuthorizationException If parsing fails.
     */
    public void parse() throws ASelectAuthorizationException
    {
        ...
        {
            _systemLoger.log(Level.WARNING, MODULE, sMethod, 
                "Error scanning authorization rule", e);
            throw new ASelectAuthorizationException(
                Errors.ERROR_ASELECT_IO);
        }
    }
     ...
     * <b>Preconditions:</b>
     * <br>
     * The evaluation tree should be parsed by calling the {@link #parse} method.
     * <br><br>
     * <b>Postconditions:</b>
     * <br>
     * -
    ...
    private EvaluationTree parseExpression() throws ASelectAuthorizationException
    {
        ...
        if(_oCurrentToken != null && 
            _oCurrentToken._iKind != AuthorizationRuleToken.GROUP_END) 
    ...
    throws ASelectAuthorizationException
    {
        ...
        else if(oToken._iKind == AuthorizationRuleToken.UNQUOTED_STRING)
        {
            ...
            if(_oCurrentToken != null && 
                _oCurrentToken._iGroup == AuthorizationRuleToken.OPERATOR_GROUP)
            {
                //accept operator token
                AuthorizationRuleToken oOperator = acceptIt();
                //accept value
	            AuthorizationRuleToken oValue = acceptGroup(
	            ...
	            if(oOperator._iKind == AuthorizationRuleToken.IN)
	            {
	                ...
	                {
	                    sb = new StringBuffer(
	                        "Unexpected token after 'in' operator: '");
	                    sb.append(oToken).append(
	                        "', in operator can only be followed by a value list.");
	                    _systemLoger.log(
	                        Level.WARNING, MODULE, sMethod,sb.toString());  
	                    throw new ASelectAuthorizationException(
	                        Errors.ERROR_ASELECT_AGENT_INTERNAL_ERROR);
	                }
	            }
	            else if(oOperator._iKind == AuthorizationRuleToken.MATCH_REGULAR_EXPRESSION)
	            {
	                ...
	                catch(IllegalArgumentException e)
	                {
	                    ...
	                    _systemLoger.log(Level.WARNING, MODULE, 
	                        sMethod,sb.toString(),e);  
	                    throw new ASelectAuthorizationException(
	                        Errors.ERROR_ASELECT_AGENT_INTERNAL_ERROR);
	                }	                
	            }
	            else
	            {
		            ...
		            oValue.setValue(AuthorizationRuleToken.convertToKnownType(
		                oValue.getValue())); 
		            //ip operator check		            
	                if(oValue.getValue() instanceof IPv4Address ||
	                    oValue.getValue() instanceof IPv6Address)
	                {
        ...
        else
        {
            ...
            sb.append(oToken).append(
                "' start of a simple expression expected.");
            _systemLoger.log(Level.WARNING, MODULE, sMethod, sb.toString());
            throw new ASelectAuthorizationException(
                Errors.ERROR_ASELECT_AGENT_INTERNAL_ERROR);
        }
     ...
     * @param tLeft the left tree of the logic expression.
     * @return A tree containing constructeded logic expressions (recursive) 
     * @throws ASelectAuthorizationException If parsing fails.
     */
    private EvaluationTree parseLogicExpression(EvaluationTree tLeft) throws ASelectAuthorizationException
    {
    ...
    private AuthorizationRuleToken acceptIt() throws ASelectAuthorizationException 
    {
		...
		{                
            _systemLoger.log(
                Level.WARNING, MODULE, sMethod, 
                "Unexpected end of authorization rule.");  
            throw new ASelectAuthorizationException(
                Errors.ERROR_ASELECT_AGENT_INTERNAL_ERROR);
        }
    	return oToken;  
	 ...
	 * @param iExpectedKind The expected kind of token.
	 ...
	 * @param iExpectedGroup The expected group type.
