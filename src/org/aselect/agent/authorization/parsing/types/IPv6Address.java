 ...
 * <code>x:x:x:x:x:x:x:x(/x:x:x:x:x:x:x:x or /x)</code>, where the 'x's are the
 ...
 * <br>
 * @author Alfa & Ariss
 * @see <a href="http://www.ietf.org/rfc/rfc2373.txt">
...
{      
     ...
     * @param s The String respresentation of the address.
     * @throws Exception If parsing fails.
     */
    public IPv6Address(String s) throws Exception
    {
        String sAdress = null;
     ...
     * @return <code>0</code> if the ip adresses are equal with 
     * network mask taken in account, otherwise -1.
     * @throws ClassCastException If the provided <code>Object</code> 
     * 	is not an <code>IPv6Address</code>.
     */
    public int compareTo(Object oOther) throws ClassCastException
    {
        ...
        if(oOther instanceof IPv6Address)
        {
            ...
            {
                bOk = (aOther._iaAddress[i] & this._iaMask[i]) 
                		== this._iaAddress[i];
            }          
        ...
        else
        {
            throw new ClassCastException(
                "IPv6Address objects are only comparable to other IPv6Address objects");
        }
        return iRet;
    }
     ...
     * @param s The address as string.
     * @param bIsMask <code>true</code> if this address is a subnet mask.
     * @return The address as a <code>int[]</code>.
     * @throws NumberFormatException If parsing fails.
     */
    private int[] convertAddress(String s, boolean bIsMask) throws NumberFormatException
    {
        int[] ia = null;
