 ...
 * <br>
 * <b>Description:</b><br>
 * The <code>LDAPProtocolHandlerFactory</code> can be used to instantiate different types of
 * <code>LDAPProtocolHandlers</code>. <br>
 * <br>
 * <b>Concurrency issues:</b> <br>
 * The created <code>ILDAPProtocolHandler</code> can be used for <u>one</u> request. <br>
 * 
 * @author Alfa & Ariss 14-11-2007 - Changes: - Added default realm when the user does not have one.
...
public class LDAPProtocolHandlerFactory
{
	...
	/**
	 * Instantiate a new <code>ILDAPProtocolHandler</code> implementation. <br>
	 * <br>
	 * <b>Description:</b> <br>
	 * Reads the context by calling {@link #getContext(Object, String, SystemLogger)} and determin the type of
	 * <code>ILDAPProtocolHandler</code>. This class is instantiated and returned. <br>
	 * <br>
	 * <b>Concurrency issues:</b> <br>
	 * - <br>
	 * <br>
		...
		throws ASelectException
	{
		...
		try {
			...
			systemLogger.log(Level.WARNING, MODULE, sMethod, "Could not instantiate LDAP protocol handler", eAS);
			throw eAS;
		}
		catch (ClassNotFoundException eCNF) {
			systemLogger.log(Level.SEVERE, MODULE, sMethod,
					"Could not instantiate LDAP protocol handler. Class was not found", eCNF);
			throw new ASelectException(Errors.ERROR_LDAP_COULD_NOT_AUTHENTICATE_USER, eCNF);
		}
		catch (Exception e) {
			systemLogger.log(Level.SEVERE, MODULE, sMethod,
					"Could not instantiate LDAP protocol handler due to internal error", e);
			throw new ASelectException(Errors.ERROR_LDAP_INTERNAL_ERROR, e);
		}
	}

	/**
	 * retrieve the context attributes of the user. <br>
	 * <br>
	 * <b>Description:</b> <br>
	 * Retrieve context attributes of the given user which are read from the configuration. <br>
	 * <br>
	 * <b>Concurrency issues:</b> <br>
	 * - <br>
	 * <br>
		...
		throws ASelectException
	{
		...
		try {
			...
			try {
				sPrincipalDn = oConfigManager.getParam(oBackendServer, "security_principal_dn");
			}
			catch (ASelectConfigException e) {
			...
			oSystemLogger.log(Level.SEVERE, MODULE, sMethod, "Could not retrieve context due to internal error", e);
			throw new ASelectException(Errors.ERROR_LDAP_INTERNAL_ERROR, e);
		}
	}
}
