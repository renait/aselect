...
public class CryptoEngine
{
     ...
     */
    public CryptoEngine(String sWorkingDir,
            AuthSPSystemLogger oAuthSPSystemLogger) throws ASelectException
    {
        ...
        try
        {
            ...
            try
            {
                oProvidersSection = _oAuthSPConfigManager.getSection(oCryptoSection,
                    "providers");
            }
            catch (ASelectConfigException eAC)
            {
            ...
            if (oProvidersSection != null)
            {
                try
	            {
	                oCryptoProvider = _oAuthSPConfigManager.getSection(oProvidersSection,
	                    "provider");
	            }
	            ...
	            while (oCryptoProvider != null) //for all providers
	            {
	                ...
	                try
	                {
	                    sProviderID = _oAuthSPConfigManager.getParam(oCryptoProvider, 
	                        "id");
	                }
	                catch (ASelectConfigException e)
	                {
	                ...
	                try
	                {
	                    sCryptoProvider = _oAuthSPConfigManager.getParam(oCryptoProvider, 
	                        "class");
	                }
	                catch (ASelectConfigException e)
	                {
	                ...
	                oCryptoProvider = _oAuthSPConfigManager
	        ...
	        {
	            _systemLogger.log(Level.CONFIG, MODULE, sMethod ,
	                "No public keys of any A-Select Server found.");
	        }
        }
        ...
        {
            _systemLogger.log(Level.SEVERE, MODULE, sMethod, 
                "Could not initialise due to internal error",e);
    ...
    throws ASelectException
    {
        ...
        catch (Exception e)
        {
            StringBuffer sbError = new StringBuffer(
                "Could not compute signature for data: \"");
            ...
            String sSignature)
    {
        ...
        {
            StringBuffer sbError = new StringBuffer(
                "Could not verify signature for data: '");
     ...
     * <code>FALSE</code> if it could not be verified.
     */
    public synchronized boolean verifyMySignature(String sData,
            String sSignature)
    {
     ...
     * keys
     */
    private void loadDefaultKeys(String sWorkingDir) throws ASelectException
    {
        ...
        try
        {
            ...
            try
            {
                oAuthSPConfig = oAuthSPConfigManager.getSection(null,
                    "authspserver");
            }
            catch (ASelectConfigException eAC)
            {
                _systemLogger.log(Level.WARNING, MODULE, sMethod,
                    "No 'authspserver' config section found", eAC);
                throw new ASelectException(Errors.ERROR_ASELECT_INIT_ERROR, eAC);
            }

            try
            {
                sPassword = oAuthSPConfigManager.getParam(oAuthSPConfig,
                    "keystore_password");
            }
            catch (ASelectConfigException eAC)
            {
                _systemLogger.log(Level.WARNING, MODULE, sMethod,
                        "No 'keystore_password' config item in 'authspserver' section found.",
                        eAC);
                throw new ASelectException(Errors.ERROR_ASELECT_INIT_ERROR, eAC);
            }
            ...
            ksPrivate.load(new FileInputStream(sbKeystoreLocation.toString()),
                null);

            char[] caPasswordChars = sPassword.toCharArray();
            _oPrivateKey = (PrivateKey)ksPrivate.getKey(KEY_ALIAS,
                caPasswordChars);
     ...
     * public keys
     */
    private void loadPublicKeys(String sWorkingDir) throws ASelectException
    {
		...
		throws ASelectException
	{
	    ...
	    if (oSection != null)
	    {
		    ...
		    try
		    {
		        _sSignatureAlgorithm = _oAuthSPConfigManager.getParam(oSection,
		            "algorithm");
		    }
		    catch (ASelectConfigException e)
		    {
