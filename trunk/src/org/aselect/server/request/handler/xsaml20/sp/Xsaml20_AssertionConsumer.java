...
//			dbFactory.setIgnoringComments(true);
			...
			String artifactResponseIssuer = ( artifactResponse.getIssuer() == null || // avoid nullpointers
						...
						"".equals(artifactResponse.getIssuer().getValue()) ) ? sASelectServerUrl : // if not in message, use sASelectServerUrl value retrieved from metadata
...
//				if (checkSignature(artifactResponse, pkey )) { // We don't need the indirection anymore
		...
		try {
			...
			if (sResultCode != null) {
				if (sResultCode.equals(Errors.ERROR_ASELECT_SERVER_CANCEL) ||
					...
					_authenticationLogger.log(new Object[] { "Cross", sUID,
							(String) htServiceRequest.get("client_ip"), sRemoteOrg,
	...
	public synchronized void setLocalityAddressRequired(
			boolean localityAddressRequired) {
