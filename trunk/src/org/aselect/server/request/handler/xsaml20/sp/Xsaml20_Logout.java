...
//	    _sLogoutResultPage = org.aselect.system.utils.Utils.replaceString(_sLogoutResultPage, "[organization_friendly]", _sFriendlyName);
	...
	throws ASelectException
	{
		...
		// TgT is gone now
		String sResultCode = ((htTGTContext==null)? Errors.ERROR_ASELECT_SERVER_UNKNOWN_TGT: Errors.ERROR_ASELECT_SUCCESS);
...
/*		if (sLogoutReturnUrl != null && !"".equals(sLogoutReturnUrl)) {
			...
			String url = sLogoutReturnUrl + sAmpQuest + "result_code=" +
				((htTGTContext==null)? Errors.ERROR_ASELECT_SERVER_UNKNOWN_TGT: Errors.ERROR_ASELECT_SUCCESS);
			try {
				_systemLogger.log(Level.INFO, _sModule, sMethod, "Redirect to "+url); 
