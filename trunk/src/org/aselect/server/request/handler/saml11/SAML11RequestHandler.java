 ...
 * <br>
 * <b>Description:</b><br>
 * Request handler for the following SAML 1.1 SSO Requests. <br>
 * <br>
...
public class SAML11RequestHandler extends AbstractRequestHandler
{
	...
	/**
	 * Initializes the SAML 1.1 Request Handler. <br>
	 * <br>
	 * <b>Description:</b><br>
	 ...
	 * <br>
	 * <ul>
	 * <li><b>clientcommunicator</b> - Client communicator used for communicating to the A-Select Server for the
	 * verify_credentials request (raw/soap11/soap12)</li>
	 * <li><b>expire</b> - The assertion expire time that must be used when creating new assertions</li>
	 * <li><b>namespace</b> - The namespace that must be used when creating an attribute statement</li>
	 * <li><b>id</b> - The A-Select app_id</li>
	 * <li><b>profile</b> - The profile that must be used for the specified app_id</li>
	 * <li><b>default</b> - The default web sso profile that will be used if an application isn't mapped to a profile</li>
	 * <br/>
	 * <li><b>storage manager</b> - The storage manager configuration is used by the A-Select Storage Manager from the
	 * A-Select System Package</li>
	 * </ul>
	 * <br>
	 * <br>
	 * 
		...
		throws ASelectException
	{
		...
		try {
			...
			try {
				...
				_systemLogger.log(Level.WARNING, MODULE, sMethod, "No config section 'websso' found", e);
				throw new ASelectException(Errors.ERROR_ASELECT_INIT_ERROR, e);
			}
			...
			try {
				...
				_systemLogger.log(Level.WARNING, MODULE, sMethod, "No config item 'default' in 'websso' section found",
						e);
				throw new ASelectException(Errors.ERROR_ASELECT_INIT_ERROR, e);
			}
			...
			try {
				...
				_systemLogger.log(Level.WARNING, MODULE, sMethod, "No config item 'class' in 'profile' section found",
						e);
				throw new ASelectException(Errors.ERROR_ASELECT_INIT_ERROR, e);
			}
			...
			while (oProfile != null) {
				...
				try {
					...
					_systemLogger.log(Level.WARNING, MODULE, sMethod,
							"Not a correct 'IWebSSOProfile' class: " + sClass, e);
					throw new ASelectException(Errors.ERROR_ASELECT_INIT_ERROR, e);
				}

				oWebSSOProfile.init(oProfile, lAssertionExpireTime, sAttributeNamespace, bSendAttributeStatement);
				if (_htWebSSOProfiles.containsKey(oWebSSOProfile.getID())) {
					_systemLogger.log(Level.WARNING, MODULE, sMethod, "profile id is not unique: "
							+ oWebSSOProfile.getID());
					throw new ASelectException(Errors.ERROR_ASELECT_INIT_ERROR);
				}
	...
	/**
	 * Processes a SAML Web SSO request. <br/>
	 * <br/>
	 * <li>Reads an A-Select <code>authenticate</code> response</li> <li>Verifies if the following paramers are
	 * available in the response:
	 * <ul>
	 * <li>aselect_credentials</li>
	 * <li>rid</li>
	 * </ul>
	 * </li> <li>Sends an A-Select <code>verify_credentials</code> API call request to the A-Select Server</li> <li>
	 * Reads the SAML session (with id: saml11_[rid]) created by the Shibboleth Authentication Profile</li> <li>
	 * Processes the request with the correct websso profile handler for the specified application</li> <br>
	 * <br>
	 * 
	 * @param request
	 *            the request
		...
		throws ASelectException
	{
		...
		try {
			...
			if (sCredentials == null) {
				_systemLogger.log(Level.WARNING, MODULE, sMethod, "No parameter 'aselect_credentials' in request");
				throw new ASelectException(Errors.ERROR_ASELECT_INTERNAL_ERROR);
			}

			String sRid = request.getParameter("rid");
			if (sRid == null) {
				_systemLogger.log(Level.WARNING, MODULE, sMethod, "No parameter 'rid' in request");
				throw new ASelectException(Errors.ERROR_ASELECT_INTERNAL_ERROR);
			}

			HashMap htResponse = handleVerifyCredentials(sCredentials, sRid, request);
			if (htResponse.isEmpty()) {
				_systemLogger.log(Level.WARNING, MODULE, sMethod, "No response after 'verify_credentials' call");
				throw new ASelectException(Errors.ERROR_ASELECT_INTERNAL_ERROR);
			}
			...
			if ((oWebSSOProfile = (IWebSSOProfile) _htWebSSOProfiles.get(sProfile)) == null) {
				_systemLogger.log(Level.WARNING, MODULE, sMethod, "No WebSSOProfile found with id: " + sProfile);
				throw new ASelectException(Errors.ERROR_ASELECT_INTERNAL_ERROR);
			}
	 ...
	 * </code> HashMap and destroys the Assertion Session Manager
	 * singleton. <br>
	 * <br>
	 * 
	...
	public void destroy()
	{
		if (_htWebSSOProfiles != null) {
			...
			/*
			 * Enumeration enumProfiles = _htWebSSOProfiles.elements(); while (enumProfiles.hasMoreElements()) {
			 * IWebSSOProfile oWebSSOProfile = (IWebSSOProfile)enumProfiles.nextElement(); oWebSSOProfile.destroy(); }
			 */
		}
	...
	/**
	 * Sends a A-Select verify_credentials request. <br>
	 * <br>
	 * 
	 * @param sCredentials
	 *            the A-Select credentials
	 * @param sRid
	 *            the A-Select rid
	 * @param request
		...
		throws ASelectException
	{
		...
		try {
			...
			if (sASelectID == null) {
				_systemLogger.log(Level.WARNING, MODULE, sMethod, "No parameter 'a-select-server' in request");
				throw new ASelectException(Errors.ERROR_ASELECT_INTERNAL_ERROR);
			}
			...
			if (!sResultCode.equals(Errors.ERROR_ASELECT_SUCCESS)) {
				_systemLogger
						.log(Level.WARNING, MODULE, sMethod, "A-Select Server returned error code: " + sResultCode);
				throw new ASelectException(Errors.ERROR_ASELECT_INTERNAL_ERROR);
			}
			return htResponse;
		}
			...
			_systemLogger.log(Level.SEVERE, MODULE, sMethod, "Could not send 'verify_credentials' request", e);
			throw new ASelectException(Errors.ERROR_ASELECT_INTERNAL_ERROR, e);
		}
	}
