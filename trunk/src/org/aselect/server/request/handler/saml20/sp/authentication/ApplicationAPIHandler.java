...
public class ApplicationAPIHandler extends AbstractAPIRequestHandler
{
	...
	long _lUpdateInterval;
	String _sSamlMessageType;
	String _sFederationUrl;
		...
		throws ASelectException
	{
		...
		String sMethod = "ApplicationAPIHandler";
		...
		_lUpdateInterval = (Long)htResult.get("update_interval");
		_sSamlMessageType = (String)htResult.get("message_type");
		_sFederationUrl = (String)htResult.get("federation_url");
	}

	/**
	 * Processes all incoming application API calls.
	 * <br><br>
	 * @see org.aselect.server.request.handler.saml20.sp.authentication.AbstractAPIRequestHandler#processAPIRequest(
		...
		throws ASelectException
	{
		...
		else if (sAPIRequest.equals("upgrade_tgt")) // Bauke: copied from aselect ApplicationAPIHandler()
		{
		...
		throws ASelectException
	{
		...
		//        Integer intMaxAppLevel = null; TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		String sAppUrl = null;
		String sAppId = null;
		//        String sASelectServer = null;	TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		Hashtable htSessionContext = null;
		//        String sUid = null;		TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		//        String sRemoteOrg = null;	TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		//        String sForcedLogon = null;	TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		//        String sCountry = null;	TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		//        String sLanguage = null;	TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		...
		try {
			...
			//            sASelectServer = oInputMessage.getParam("a-select-server");	TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		}
		catch (ASelectCommunicationException eAC) {
		...
		/*	
		 ...
		 if (sUid != null)
		 sbData.append(sUid);
		 verifyApplicationSignature(oInputMessage, null, sbData.toString(), sAppId);
		 }
		 <<<<<<<<<<<<<<<<<<<<<<<<<<
		 */
		...
		//        intMaxAppLevel = _applicationManager.getMaxLevel(sAppId); TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		...
		//        if(intMaxAppLevel != null)	TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		//            htSessionContext.put("max_level", intMaxAppLevel); TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		...
		/*	
		 ...
		 _applicationManager.isForcedAuthenticateEnabled(sAppId))
		 {
		...
		//        Vector vAuthSPs = _authSPManagerManager.getConfiguredAuthSPs(intAppLevel, intMaxAppLevel); TODO Volgens mij gebruiken we dit niet, en kan dus weg 10-10-07 HW
		Vector vAuthSPs = _authSPManagerManager.getConfiguredAuthSPs(intAppLevel);
	 ...
	 */
	private void verifyApplicationSignature(IInputMessage oInputMessage, String sArgSignature, String sData, String sAppId)
		throws ASelectException
	{
        ...
        if (oInputMessage == null) {
            sSignature = sArgSignature;  // was already read by the caller
            if (sSignature == null) {
		...
		throws ASelectException
	{
		...
		String errorCode = reallyHandleTgtUpgrade(sEncTGT, sASelectServer, sSignature);

        // Report to the outside world
		try {
			oOutputMessage.setParam("result_code", errorCode);
		}
		catch (ASelectCommunicationException eAC) {
			_systemLogger.log(Level.WARNING, _sModule, sMethod, "Could not set response parameter", eAC);
			throw new ASelectCommunicationException(Errors.ERROR_ASELECT_INTERNAL_ERROR, eAC);
    ...
    // Needs aselect_server and signature to check signing
	private String reallyHandleTgtUpgrade(String sEncTGT, String sASelectServer, String sSignature)
		throws ASelectCommunicationException, ASelectException
	{
        String sMethod = "reallyHandleTgtUpgrade()";
		String sTGT = null;
		try {
			byte[] baTgtBlobBytes = CryptoEngine.getHandle().decryptTGT(sEncTGT);
			sTGT = Utils.toHexString(baTgtBlobBytes);
		...
		if (_applicationManager.isSigningRequired()) {
			...
			String sAppId = (String) htTGTContext.get("app_id");
			StringBuffer sbData = new StringBuffer(sASelectServer).append(sEncTGT);
			verifyApplicationSignature(null, sSignature, sbData.toString(), sAppId);
		}
		...
		SessionSyncRequestSender ss_req = new SessionSyncRequestSender(_systemLogger, _sRedirectUrl,
				_lUpdateInterval, _sSamlMessageType, _sFederationUrl);
		_systemLogger.log(Level.INFO, _sModule, sMethod, "Check sync");
		errorCode = ss_req.synchronizeSession(sTGT, false, false);  // credentials not crypted, no ticket upgrade
		return errorCode;
	}
}
