...
public class ApplicationBrowserHandler extends AbstractBrowserRequestHandler
{
	...
	// TODO add private SessionContext _sessionContext;
	...
	private final String _sUID = "digid_user"; // Dummy user_id to bypass the window for user_id
		...
		throws ASelectException
	{
		...
		_systemLogger.log(Level.INFO, _sModule, sMethod, "ApplBrowREQ");
		...
		if (sRequest == null) {
			...
			// whole servlet request handling.
			_htSessionContext = _sessionManager.getSessionContext(sRid);
			// TODO add _sessionContext =
			// _saml20SessionManager.getSessionContext(sRid);

			if (_htSessionContext == null)
			// TODO add if (_sessionContext == null)
			{
			...
			// TODO add String sDirectAuthSP =
			// _sessionContext.getDirectAuthSP();
	 ...
	 * <br>
	 * <b>Description:</b> <br>
	 * All the requests which contains direct_login (e.g. direct_login1,
	 * direct_login2) will be handled by this function and send to the direct
	 * authsp handler. <code>request=cross_login</code> instead. <br>
	 * <br>
	 ...
	 * @throws ASelectException
	 */
	private void handleDirectLogin(Hashtable htServiceRequest, HttpServletResponse servletResponse, PrintWriter pwOut)
		throws ASelectException
	{
		...
		try {
			...
			if (_configManager.isSingleSignOn() && htServiceRequest.containsKey("aselect_credentials_tgt")
				...
				// match
				{
					...
					if (checkCredentials(sTgt, sUid, sServerId)) // valid
					// credentials/level/SSO
					// group
					{
						...
						if (!boolForced.booleanValue()) {
							...
							if (_htSessionContext.get("remote_session") == null) {
								// sRedirectUrl = (String)
								// _htSessionContext.get("app_url");
								sRedirectUrl = (String) _htSessionContext.get("assertion_consumer_service_url");
							}
							else {
							...
							oTGTIssuer.sendSAMLArtifactRedirect(sRedirectUrl, sRid, _htSessionContext, sTgt,
									htTGTContext, servletResponse);
					...
					// User was originally authenticated at this A-Select
					// Server
	 ...
	 * redirected back to the application (or the local A-Select Server in case
	 * of cross A-Select).<br>
	 * If no valid TGT is found, the user is presented an HTML page asking for a
	 * user name. The HTML page implements a HTML-form that will POST a
	 * <code>request=login2</code>. If the user is not from this organizatio,
	 * the HTML-form might POST a <code>request=cross_login</code> instead.
	 * <br>
	 * <br>
	 * <b>Concurrency issues:</b> <br> - <br>
	 * <br>
		...
		throws ASelectException
	{
		...
		try {
				...
				// user_id does not match
				{
					...
					if (checkCredentials(sTgt, sUid, sServerId))
					{
						...
						if (!boolForced.booleanValue()) {
							...
							// add the SP to to SSO administration
							SSOSessionManager ssoSessionManager = SSOSessionManager.getHandle();
							UserSsoSession ssoSession = ssoSessionManager.getSsoSession(sUid);
							String serviceProviderUrl = (String) _htSessionContext.get("sp_issuer");
							ServiceProvider sp = new ServiceProvider();
							sp.setServiceProviderUrl(serviceProviderUrl);
							sp.setLastSessionSync(new Date().getTime());
							ssoSession.addServiceProvider(sp);
							// overwrite session (needed for database storage)
							...
							oTGTIssuer.sendSAMLArtifactRedirect(sRedirectUrl, sRid, _htSessionContext,
									sTgt, htTGTContext,	servletResponse);
							_sessionManager.killSession(sRid);
							return;
						}
					}
					...
					// Authenicate with same user-id that was stored in TGT
					Hashtable htTGTContext = _tgtManager.getTGT(sTgt);
					// Signal that we intend to upgrade the current ticket
					// Needed to prevent login refusal by AuthSPBrowserHandler().handleAuthSPResponse()
					_htSessionContext.put("upgrade_tgt", sTgt);
			...
			// no TGT found or killed (other uid)
			_systemLogger.log(Level.INFO, _sModule, sMethod, "login consent available");
			if (!_configManager.isUDBEnabled() || _htSessionContext.containsKey("forced_organization")) {
				_systemLogger.log(Level.INFO, _sModule, sMethod, "To CROSS2");
				handleCrossLogin(htServiceRequest, servletResponse, pwOut);
				return;
			}
			String sForcedUid = (String) _htSessionContext.get("forced_uid");
			...
			handleLogin2(htServiceRequest, servletResponse, pwOut);
			_systemLogger.log(Level.INFO, _sModule, sMethod, "Login2 done, no login Form");

			/*
			 * // show login (user_id) form, but we don't want that, do we?
			 * sLoginForm = Utils.replaceString(sLoginForm, "[rid]", sRid);
			 * sLoginForm = Utils.replaceString(sLoginForm, "[aselect_url]",
			 * (String) htServiceRequest.get("my_url")); sLoginForm =
			 * Utils.replaceString(sLoginForm, "[a-select-server]",
			 * _sMyServerId); sLoginForm = Utils.replaceString(sLoginForm,
			 * "[request]", "login2"); sLoginForm =
			 * Utils.replaceString(sLoginForm, "[cross_request]",
			 * "cross_login"); sbUrl = new StringBuffer((String)
			 * htServiceRequest.get("my_url")).append("?request=error").append("&result_code=").append(Errors.ERROR_ASELECT_SERVER_CANCEL).append(
			 * "&a-select-server=").append(_sMyServerId).append("&rid=").append(sRid);
			 * sLoginForm = Utils.replaceString(sLoginForm, "[cancel]",
			 * sbUrl.toString()); sLoginForm =
			 * _configManager.updateTemplate(sLoginForm, _htSessionContext);
	...
	//
	private boolean handleUserConsent(Hashtable htServiceRequest, HttpServletResponse servletResponse,
	...
	throws ASelectException
	{
		...
		try {
			...
			sFriendlyName = getSimpleParam(aselect, "organization_friendly_name", false);
			String sAskConsent = getSimpleParam(aselect, "request_consent", false);
		...
		try {
			_sConsentForm = org.aselect.server.request.handler.saml20.common.Utils.loadHTMLTemplate(_systemLogger, _configManager.getWorkingdir(), "userconsent.html");    
		    ...
		    StringBuffer sCancel = new StringBuffer(sAsUrl).append("?request=login1")
		    		.append("&rid=").append(sRid).append("&a-select-server=").append(_sMyServerId)
		    		.append("&consent=false");
	...
	// Bauke: copied from ProtoRequestHandler
	protected String getSimpleParam(Object oConfig, String sParam, boolean bMandatory)
	throws ASelectException
	{
		try {
			...
			_systemLogger.log(Level.WARNING, _sModule, "getSimpleParam", "No config item '"+sParam+"' found", e);
	 ...
	 * list in a HTML page. This HTML page will POST a
	 * <code>request=login3</code>.<br>
	 * Depending on the A-Select configuration
	 * <code>always_show_select_form</code> the request can be parsed to
	 * <code>handleLogin3()</code> immediately if only one valid AuthSP is
	 * found. <br>
	 * <br>
	 * <b>Concurrency issues:</b> <br> - <br>
	 * <br>
		...
		throws ASelectException
	{
		...
		_systemLogger.log(Level.INFO, _sModule, sMethod, "Login2");
		StringBuffer sb;
		try {
			sRid = (String) htServiceRequest.get("rid");
			...
			_systemLogger.log(Level.INFO, _sModule, sMethod, "Allowed AuthSP's=" + htAuthsps);
			if (htAuthsps.size() == 1) {
				try {
					...
					if (sFormShow.equalsIgnoreCase("false")) {
						...
						htServiceRequest.put("authsp", htAuthsps.keys().nextElement());
	 ...
	 * @throws ASelectException
	 */
	private void handleLogin3(Hashtable htServiceRequest, HttpServletResponse servletResponse, PrintWriter pwOut)
		throws ASelectException
	{
		String sRid = null;
		String sAuthsp = null;
		...
		throws ASelectException
	{
		...
		try {
			...
			_sessionManager.put(sLocalRid, _htSessionContext);  // storage_manager_fix_for_lost_fields
			...
			_systemLogger.log(Level.INFO, _sModule, sMethod, "XLOGIN htRequestTable=" + htRequestTable + " to "
					+ sRemoteAsUrl);
			...
			try {
				...
				_systemLogger.log(Level.INFO, _sModule, sMethod, "Redirect to " + sbUrl);
	 ...
	 */
	private void handleIPLogin1(Hashtable htServiceRequest, HttpServletResponse servletResponse, PrintWriter pwOut)
		throws ASelectException
	{
		...
		try {
			...
			// TODO IP login is not used when a user already has a TGT
			// (Peter)
			// IP login is not used when a user already has a TGT. The
			// origin
			// ip-range will never be forced when already authenticated with
			// an
				...
				if (checkCredentials(sTgt, sUid, sServerId)) {
					...
					if (_htSessionContext.get("cross_authenticate") != null) {
						...
						if (_htSessionContext.get("cross_authenticate") != null) {
							...
							// TGTIssuer will redirect to local A-Select
							// Server

							// TODO Check if a new TGT must be created
							// (Peter)
							// A new TGT is created because the TGTIssuer
							// implements the redirect with a create
							// signature.
							...
							// issue new one but with the same lifetime as
							// the
							// existing one
							Hashtable htAdditional = new Hashtable();
							// FIXME StorageManager can't update timestamp,
							// this doesn't work.
							// (Erwin, Peter)
					...
					try {
						...
						_sessionManager.killSession(sRid);
						_systemLogger.log(Level.INFO, _sModule, sMethod, "Redirect to " + sb);
		...
		throws ASelectException
	{
		...
		try {
			...
			if (htTGTContext != null) {
				...
				_systemLogger.log(Level.INFO, _sModule, sMethod, "Delete Cookie=" + cKillCookie.getName() + " domain="
						+ sCookieDomain);
				...
				if (sRemoteAsUrl != null) {
					_systemLogger.log(Level.INFO, _sModule, sMethod, "Redirect to " + sRemoteAsUrl);
		...
		throws ASelectException
	{
		...
		try {
			...
			_systemLogger.log(Level.INFO, _sModule, sMethod, "userProfile="+htUserProfile);
			...
			_systemLogger.log(Level.INFO, _sModule, sMethod, "required="+intRequiredLevel+" max="+intMaxLevel);
			...
			if (vAllowedAuthSPs == null) {
				_systemLogger.log(Level.WARNING, _sModule, sMethod, "No Allowed AuthSP's for" + sUid);
				throw new ASelectException(Errors.ERROR_ASELECT_INTERNAL_ERROR);
			}
	...
	private String startAuthentication(String sRid, Hashtable htLoginRequest)
		throws ASelectException
	{
		Hashtable htAllowedAuthsps;
		String sAuthsp = null;
		String sMethod = "startAuthentication()";

		sAuthsp = (String) htLoginRequest.get("authsp");

		htAllowedAuthsps = (Hashtable) _htSessionContext.get("allowed_user_authsps");
		...
		Hashtable htResponse = oProtocolHandler.computeAuthenticationRequest(sRid);
	 ...
	 * required level for the current application. <br>
	 * 
	 * @param sTgt
	...
	private boolean checkCredentials(String sTgt, String sUid, String sServerId)
	{
		...
		intRequiredLevel = org.aselect.server.request.handler.saml20.common.Utils
				.getIntBetrouwbaarheidsNiveauFromBN((String) _htSessionContext
						.get("requested_betrouwbaarheidsniveau"));

		// TODO remove sTGTLevel = (String) htTGTContext.get("authsp_level");
		sTGTLevel = (String) htTGTContext.get("betrouwbaarheidsniveau");
