...
public class SAML20SPLogoutRequestHandler extends AbstractRequestHandler
{
	private final static String _sModule = "SAML20SPLogoutRequestHandler";
	...
	/**
	 * Init for class SAML20SPLogoutRequestHandler. <br>
	 * 
		...
		throws ASelectException
	{
		...
		try {
			...
			sTGT = org.aselect.system.utils.Utils.toHexString(baTgtBlobBytes);
		}
		...
		if (!_oTGTManager.containsKey(sTGT)) {
			...
			try {
				...
				String sHtmlPage = org.aselect.system.utils.Utils.replaceString(_sLogoutResultPage, "[result_code]", Errors.ERROR_ASELECT_SERVER_UNKNOWN_TGT);
...
*/
		...
		cookie.setMaxAge(0);
		_systemLogger.log(Level.INFO, _sModule, sMethod, "Delete Cookie=" + SpTGTIssuer.COOKIE_NAME);
		...
		logoutRequestSender.sendLogoutRequest(url, _sReturnUrl, user, request, response, "user initiated logout");
	}
